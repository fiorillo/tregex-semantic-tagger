...
public class WordToSentenceProcessor extends AbstractListProcessor {
  ...
  public List process(List words) {
    ...
    for (Iterator iter = words.iterator(); iter.hasNext();) {
      ...
      if (DEBUG) {
        EncodingPrintWriter.err.println("Word is " + w, "UTF-8");
      }
      ...
      if (sentenceBoundaryFollowers.contains(w) && lastSentence != null && currentSentence.isEmpty()) {
        lastSentence.add(o);
        if (DEBUG) {
  ...
  public WordToSentenceProcessor() {
    this(new HashSet<String>(Arrays.asList(new String[]{".", "?", "!"})));
  }
  ...
  public WordToSentenceProcessor(Set boundaryTokens) {
    this(boundaryTokens, new HashSet<String>(Arrays.asList(new String[]{")", "]", "\"", "\'", "''", "-RRB-", "-RSB-"})));
  }
