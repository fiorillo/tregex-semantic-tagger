...
public final class SloppyMath {
  ...
  public static double poisson(int x, double lambda) {
    ...
    double p = (Math.exp(-lambda) * Math.pow(lambda, (double) x)) / factorial(x);
    if (Double.isInfinite(p) || p<=0.0) throw new RuntimeException(Math.exp(-lambda) +" "+ Math.pow(lambda, (double) x) +" "+ factorial(x));
    return p;
  }
