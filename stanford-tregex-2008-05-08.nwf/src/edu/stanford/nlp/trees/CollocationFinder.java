...
public class CollocationFinder {
  ...
  /**
   * Construct a new <code>CollocationFinder</code> over the 
   * <code>Tree</code> t.
  ...
  /**
   * Construct a new <code>CollocationFinder</code> over the <code>Tree</code> t.
   * @param t parse tree
   * @param w w
  ...
  public CollocationFinder(Tree t, WordNetConnection w, HeadFinder hf) {
    ...
    this.collocationCollector = new ArrayList<Collocation>();
    this.hf=hf;
 ...
 *
 * @return a {@link List} of lists of the indices of collocation constituents.
 */
  ...
  private static String treeAsStemmedCollocation(Tree t) {
    ArrayList<WordTag> list= getStemmedWordTagsFromTree(t);
  ...
  private static String treeAsNonStemmedCollocation(Tree t) {
    ArrayList<WordTag> list= getNonStemmedWordTagsFromTree(t);
    StringBuilder s=new StringBuilder(160);
    WordTag firstWord = list.remove(0);
    s.append(firstWord.word());
  ...
  private static String mergeLeavesIntoCollocatedString(Tree t) {
    ...
    Sentence sent = t.taggedYield();
    for (int i =0;i<sent.size();i++) {
      sb.append((sent.getHasWord(i).word())+"_");
    }
  ...
  private static ArrayList<WordTag> getStemmedWordTagsFromTree(Tree t) {
    ArrayList<WordTag> stemmedWordTags = new ArrayList<WordTag>();
    Sentence s = t.taggedYield();
    for(int i = 0; i < s.size(); i++) {
      TaggedWord w = (TaggedWord) s.getHasWord(i);
