...
public class WordTag implements Label, HasWord, HasTag, Comparable<WordTag> {
  ...
  // TODO: Make this get both tag and word of appropriate type
  // but AcronymLexicalResource is one place that relies on current 
  // behavior.
  ...
  public int compareTo(WordTag wordTag) {
    int first = word().compareTo(wordTag.word());
    if(first != 0)
      return first;
    else {
      if (tag() == null) {
