...
public class DocumentReader {
  ...
  public DocumentReader(Reader in) {
    this(in, PTBTokenizer.PTBTokenizerFactory.newPTBTokenizerFactory(), true);
  }
  ...
  protected Document parseDocumentText(String text) {
    return (new BasicDocument().init(text, keepOriginalText));
  }
  ...
  public static String readText(Reader in) throws IOException {
    ...
    StringBuffer sb = new StringBuffer(16000);  // make biggish
    int c;
    while ((c = br.read()) >= 0) {
      sb.append((char) c);
    }
   ...
   * If <tt>in</tt> is a <tt>File</tt>, <tt>String</tt>, <tt>URL</tt>,
   * <tt>InputStream</tt>, or <tt>Reader</tt>, a Reader is opened for it
   * using one of the other static <tt>getReader</tt> methods of this class.
   * NOTE: If <tt>in</tt> is a <tt>String</tt> it is treated as a Filename, not a
   * text source as with {@link #getReader(String)}. If <tt>in</tt> is not one
   * of these file types, <tt>null</tt> is returned.
   */
  public static Reader getReader(Object in) throws FileNotFoundException, IOException {
    if (in instanceof File) {
      return (getReader((File) in));
    }
    if (in instanceof String) {
      return (getReader(new File((String) in)));
    }
    if (in instanceof URL) {
      return (getReader((URL) in));
    }
    if (in instanceof InputStream) {
      return (getReader((InputStream) in));
    }
