...
public class ExhaustivePCFGParser implements Scorer, KBestViterbiParser {
  ...
  public boolean parse(LatticeReader lr) { // ExhaustivePCFGParser
    ...
    // map to words
    List lWords = lr.getLatticeWords();
    words = new int[lWords.size()];
    offsets = new IntPair[lWords.size()];
    int unk = 0;
    StringBuilder unkWords = new StringBuilder("[");
    for (int i = 0, size = lWords.size(); i < size; i++) {
      String s = ((LatticeReader.LatticeWord) lWords.get(i)).word;
      if (Test.verbose && !lex.isKnown(wordNumberer.number(s))) {
        unk++;
        unkWords.append(" ");
        unkWords.append(s);
      ...
      /*
      if (o instanceof HasTag) { // collect the tags of the input sentence
        String tag = ((HasTag)o).tag();
        if(tagNumberer.hasSeen(tag))
          inputTags[i] = tagNumberer.number(tag);
      }
      */
    }
    // initialize inside and outside score arrays
    ...
    if (Test.verbose) {
      Timing.tick("done.");
      unkWords.append(" ]");
      op.tlpParams.pw().println("Unknown words: " + unk + " " + unkWords);
    ...
    bestScore = iScore[0][length][goal];
    boolean succeeded = hasParse();
    if (Test.doRecovery && bestScore == Float.NEGATIVE_INFINITY && !floodTags) {
      floodTags = true; // sentence will try to reparse
      System.err.println("Trying recovery parse...");
      return parse(lr);
    }
...
oScore[split][end][br.rightChild] = totR;
  ...
  private void initializeChart(LatticeReader lr) {
    ...
    for (LatticeReader.LatticeWord lWord : latticeWords) {
      int start = lWord.startNode, end = lWord.endNode;
      ...
      for (int state = 0; state < numStates; state++) {
        if (isTag[state]) {
          ...
          // ignoring LatticeReader's language model score
          //        float lmS = (float) lWord.lm;
          // the position passed into lex shouldn't really be start here, but
          // lets leave it for now
          float newScore = lex.score(itw, start) + (float) lWord.am;
      ...
      if (floodTags && (!Test.noRecoveryTagging)) { // sends to recovery parse with more flexible tagging
        for (int state = 0; state < numStates; state++) {
          float iS = iScore[start][end][state];
  ...
  } // end initializeChart(LatticeReader)
    ...
    if (end - start <= Test.maxSpanForTags && tagNumberer.hasSeen(goalStr)) {
      if (Test.maxSpanForTags > 1) {
        ...
        if (sentence != null) {
          ...
          List<LatticeReader.LatticeWord> latticeWords = lr.getWordsOverSpan(start, end);
          for (LatticeReader.LatticeWord lWord : latticeWords) {
            IntTaggedWord itw = new IntTaggedWord(wordNumberer.number(lWord.word), Numberer.translate(stateSpace, "tags", goal));
            float tagScore = lex.score(itw, start);
            if (matches(bestScore, tagScore + (float) lWord.am)) {
              wordNode = tf.newLeaf(new StringLabel(lWord.word));
              break;
            }
          }
          if (wordNode == null) {
