...
public abstract class GrammarCompactor {
  ...
  public Pair<UnaryGrammar,BinaryGrammar> compactGrammar(Pair<UnaryGrammar,BinaryGrammar> grammar) {
    return compactGrammar(grammar, new HashMap(), new HashMap());
  }
  ...
  public Pair<UnaryGrammar,BinaryGrammar> compactGrammar(Pair<UnaryGrammar,BinaryGrammar> grammar, Map allTrainPaths, Map allTestPaths) {
    ...
    for (Iterator graphIter = graphs.entrySet().iterator(); graphIter.hasNext();) {
      ...
      trainPaths = (List) allTrainPaths.remove(cat);// to save memory
      if (trainPaths == null) {
        trainPaths = new ArrayList();
      }
      testPaths = (List) allTestPaths.remove(cat);// to save memory
      if (testPaths == null) {
  ...
  protected Distribution computeInputPrior(Map trainPathMap) {
    ...
    for (Iterator catI = trainPathMap.values().iterator(); catI.hasNext();) {
      List pathList = (List) catI.next();
      for (Iterator pathI = pathList.iterator(); pathI.hasNext();) {
        List path = (List) pathI.next();
        for (Iterator inputI = path.iterator(); inputI.hasNext();) {
  ...
  public Map convertGrammarToGraphs(Pair<UnaryGrammar,BinaryGrammar> grammar, Set unaryRules, Set binaryRules) {
    ...
    for (Iterator ruleIter = bg.iterator(); ruleIter.hasNext();) {
      BinaryRule rule = (BinaryRule) ruleIter.next();
      numRules++;
    ...
    for (Iterator ruleIter = ug.iterator(); ruleIter.hasNext();) {
      UnaryRule rule = (UnaryRule) ruleIter.next();
      numRules++;
  ...
  public Pair<UnaryGrammar,BinaryGrammar> convertGraphsToGrammar(Set graphs, Set<UnaryRule> unaryRules, Set<BinaryRule> binaryRules) {
    ...
    for (Iterator graphIter = graphs.iterator(); graphIter.hasNext();) {
      TransducerGraph graph = (TransducerGraph) graphIter.next();
      Object startNode = graph.getStartNode();
      for (Iterator arcIter = graph.getArcs().iterator(); arcIter.hasNext();) {
    ...
    if (outputType == RAW_COUNTS) {
      ...
      for (UnaryRule rule : unaryRules) {
        symbolCounter.incrementCount(newStateNumberer.object(rule.parent), rule.score);
      }
      for (BinaryRule rule : binaryRules) {
        symbolCounter.incrementCount(newStateNumberer.object(rule.parent), rule.score);
      }
    ...
    for (UnaryRule rule : unaryRules) {
      if (outputType == RAW_COUNTS) {
        double count = symbolCounter.getCount(newStateNumberer.object(rule.parent));
