...
public class EnglishTreebankParserParams extends AbstractTreebankParserParams {
  ...
  protected class EnglishSubcategoryStripper implements TreeTransformer {
    ...
    public Tree transformTree(Tree tree) {
      ...
      if (tree.isLeaf()) {
        ...
        if (EnglishTest.retainADVSubcategories && s.indexOf("-ADV") >= 0) {
          s = tlp.basicCategory(s);
          s += "-ADV";
        } else if (EnglishTest.retainTMPSubcategories && s.indexOf("-TMP") >= 0) {
          s = tlp.basicCategory(s);
          s += "-TMP";
  ...
  public Lexicon lex(LexOptions op) {
    if(op.uwModel == null) {
      ...
      op.uwModel = new EnglishUnknownWordModel(op);
    }
  ...
  public int setOptionFlag(String[] args, int i) {
    //first, see if it's a generic option
