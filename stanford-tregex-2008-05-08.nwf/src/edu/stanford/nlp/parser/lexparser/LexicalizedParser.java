 ...
 * <p>
 * Note that training on a 1 million word treebank requires a lot of
...
public class LexicalizedParser implements ViterbiParser, Function<Object,Tree> {
  ...
  // this should not really be static but for the moment it is, because I don't want
  // to track down all the places that i'd have to thread it down into -- Roger
  static boolean basicCategoryTagsInDependencyGrammar = false;
  static String trainTreeFile = null; // same for me -- Teg
  ...
  public final ParserData getParserDataFromTreebank(Treebank trainTreebank,
    ...
    Extractor dgExtractor = op.tlpParams.dependencyGrammarExtractor(op);
    ...
    if (trainTreeFile!=null) {
      try {
        ...
        IOUtils.writeObjectToFile((Serializable) binaryTrainTrees, trainTreeFile);
        Timing.tick("done.");
      } catch (Exception e) {
  ...
  protected final ParserData getParserDataFromTreebank(Treebank trainTreebank, DiskTreebank secondaryTrainTreebank, double weight, GrammarCompactor compactor) {
    ...
    // BUG: With this implementation the lexicon is always trained with the
  ...
  public double testOnTreebank(Treebank testTreebank) {
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("pcfgTA"))) {
      pcfgTA = new AbstractEval.TaggingEval("pcfg Tag", runningAverages, pd.lex);
    }
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("depTA"))) {
      depTA = new AbstractEval.TaggingEval("dep Tag", runningAverages, pd.lex, true);
    }
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("factTA"))) {
      factTA = new AbstractEval.TaggingEval("factor Tag", runningAverages, pd.lex);
    }
    ...
    AbstractEval kGoodLB = new LabeledConstituentEval("kGood LP/LR", false, tlp);
    ...
    for (Tree goldTree : testTreebank) {
      ...
      Sentence s = getInputSentence(goldTree);
      timer.start();
      ...
      if (tree != null) {
        ...
        if (factTA != null) {
          // note that this gives bad results if tree is really from dep parser
        ...
        if (factCB != null) {
          factCB.evaluate(tc.transformTree(tree), transGoldTree, pwErr);
        }
        if (Test.evalb) {
    ...
    if (tsv) {
      ...
      pwErr.print("\t");
      if (factTA != null) pwErr.print(nf.format(factTA.getEvalbF1Percent()));
      pwErr.print("\t");
   ...
   * before any other language-specific options that are used.
  ...
  public static void main(String[] args) {
    ...
    while (argIndex < args.length && args[argIndex].charAt(0) == '-') {
      if (args[argIndex].equalsIgnoreCase("-train") ||
        ...
        argIndex += 2;
      } else if (args[argIndex].equalsIgnoreCase("-useBasicCategoryTagsInDependencyGrammar")) {
        LexicalizedParser.basicCategoryTagsInDependencyGrammar = true;
        argIndex++;
    ...
    if (Test.verbose || train) {
      ...
      //if (Test.verbose) {
        ...
        System.err.println("States are: " + Numberer.getGlobalNumberer("states"));
