...
//  buttonBox.add(emptyLabel2, buttonConstraints);
  ...
  public StringBuffer getHistoryString() {
  ...
  public void useProgressBar(boolean useProgressBar) {
    if(useProgressBar) {//make sure we're in progress bar mode
      if(progressBar == null) {
        ...
        Container parent = foundStats.getParent();
        useProgressBarHelper(parent, progressBar, foundStats);
      }
    } else {//make sure we're in found stats mode
      if(progressBar != null) {
        Container parent = progressBar.getParent();
        useProgressBarHelper(parent, foundStats, progressBar);
        progressBar = null;
      }
  ...
  private void useProgressBarHelper(Container parent, JComponent add, JComponent remove) {
    GridBagConstraints c = new GridBagConstraints();
    c.fill = GridBagConstraints.BOTH;
    c.weightx = 1.7;
  ...
  private void showHistory() {
    ...
    DefaultTableModel tableModel = new DefaultTableModel(entries, HistoryEntry.columnNamesArray()) {
    ...
    searchThread = new Thread() {
      public void run() {
        try {
          ...
          doError("Sorry, there was an error compiling the Tsurgeon script.  Please press Help if you need assistance.");
          SwingUtilities.invokeLater(new Runnable() {
            public void run() {
              setTregexState(false);
  ...
  private TRegexGUITreeVisitor getMatchTreeVisitor(String patternString, Thread t) {
    ...
    try {
      ...
      doError("Sorry, there was an error compiling the Tregex pattern.  Please press Help if you need assistance.");
    }
    return vis;
  }
   ...
   */
  public void doError(final String txt) {
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        JOptionPane.showMessageDialog(InputPanel.this, txt, "Compile Error", JOptionPane.ERROR_MESSAGE);
        useProgressBar(false);
  ...
  "<h1>TSurgeon Script Syntax and Uses</h1><p>Tsurgeon is a tool that can modify the trees found by Tregex searches.  These modifications are specified via a script language, and the script language allows one to, among other operations, rename, delete, move, and insert nodes in a tree.  " +
  " For example, if you want to excise an SBARQ node whenever it is the parent of an SQ node, and rename the SQ node to S, your input would look like this:" +
  "<p>" +
  "Tregex pattern: " +
  " <blockquote>" +
  " <code>" +
  ...
  " <dl>" +
  "   <dt><code>delete &#60;name&#62;</code></dt>  <dd>deletes the node and everything below it.</dd>" +
  ...
  " name1 to name2.  All the children of name2 go into the parent of name1, where name1 was.</dd>" +
  " <dt><code>relabel &#60;name&#62; &#60;new-label&#62;</code></dt> <dd>relabels the node to have the new label.</dd>" +
  " <dt><code>insert &#60;name&#62; &#60;position&#62;</code></dt> <dd>inserts the named node into the position specified. (giving a tree rather than a node is also valid)</dd>" +
  ...
  "      <dd><code>&gt;i</code> the i_th daughter of the named node</dd>" +
  "      <dd><code>&gt;-i</code> the i_th daughter, counting from the right, of the named node.</dd></dl></dd>" +
  ...
  " <dt>coindex &#60;name_1&#62; &#60;name_2&#62; ... &#60;name_m&#62; </code></dt>  <dd>Puts a (Penn Treebank style) coindexation suffix of the form \"-N\" on" +
  ...
  "after being applied.  This can cause cycles if your Tregex pattern continues to match the same " +
  ...
  "Tregex GUI seems to be stuck in an infinite loop, press Cancel to stop the operation and recheck your Tregex pattern " +
  ...
  private static final String htmlHelp = "<html><h1>Tregex Pattern Syntax and Uses</h1><p> Using a Tregex pattern, you can find only those trees that match the pattern you're " +
  "looking for.  The following table shows the symbols that are allowed in the pattern, and below there is more information " +
  ...
  " </em> Label description regular expressions are matched as <code>find()</code>," +
  " as in Perl/tgrep;" +
  ...
  " name to retreived the matched node using <code>TregexMatcher#getNode(Object o)</code>" +
  ...
  " </blockquote>" +
  " will match only such that the WH- node under the SBAR is coindexed with the trace node that gets the name <code>empty</code>.</html>";
}
