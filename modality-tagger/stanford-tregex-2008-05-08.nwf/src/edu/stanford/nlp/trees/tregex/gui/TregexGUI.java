...
public class TregexGUI extends JFrame implements ActionListener, MatchesPanelListener {
  ...
  private void initAboutBox() {
    ...
    aboutBox.getContentPane().add(new JLabel("<html><b>Tregex and Tsurgeon</b></html>", JLabel.CENTER), BorderLayout.NORTH);

    aboutBox.getContentPane().add(new JLabel("<html>Tregex by Galen Andrew and Roger Levy<br>Tsurgeon by Roger Levy<br>Graphical interface by Anna Rafferty<br>Additional features and development by Chris Manning<br></html>", JLabel.CENTER), BorderLayout.CENTER);
    aboutBox.getContentPane().add(new JLabel("<html><font size=2>\u00A92007 The Board of Trustees of The Leland Stanford Junior University.<br>Distributed under the GNU General Public License</font></html>", JLabel.CENTER), BorderLayout.SOUTH);
   ...
   * @return
   */
  private JSplitPane setUpTopPanels() {
...
//Creates a new JFileChooser, doing the boilerplate
  ...
  private void doLoadFiles() {
    ...
    if (status == JFileChooser.APPROVE_OPTION) {
      //now set up the file filters
      doFileFilters(chooser.getSelectedFiles());
      //System.err.println("Opening files " + cFile.toString());
  ...
  private void doFileFilters(File[] files) {
    ...
    JLabel text = new JLabel("<html>Please indicate any constraints on the types of files you would like to load (press okay to load all).  Only files that comply with all given constraints will be loaded.</html>");
    ...
    okay.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent arg0) {
        ...
        final HashMap<FilterType,String> filters = getFilters(fileFilterPanel);
  ...
  private HashMap<FilterType,String> getFilters(JPanel panel) {
    ...
    for(Component c : components) {
      ...
      JComboBox filterType = (JComboBox) ((Box) c).getComponent(0);
      JTextField filterValue = (JTextField) ((Box) c).getComponent(2);
    ...
    if(status == JFileChooser.APPROVE_OPTION) {
      Thread t = new Thread() {
        public void run() {
          try {
            BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(chooser.getSelectedFile()), FileTreeModel.getCurEncoding()));
            StringBuffer sb = InputPanel.getInstance().getHistoryString();
            out.write(sb.toString());
            out.flush();
            out.close();
          } catch(Exception e) {
