...
public class PennTreeReader implements TreeReader {
  ...
  private Tree readTreeHelper() throws IOException {
    wordIndex = 0;
    Tree tr = readTree((String) st.next());
    if (tr == null || tn == null) {
      return tr;
    } else {
  ...
  private Tree readTree(String token) throws IOException {
    if (DEBUG) {
      System.err.println("readTree() next token " + token);
    }
    String name;
    ...
    if (token == null) {
      ...
      name = (String) st.peek();
      if (DEBUG) {
        System.err.println("  peeked is \"" + name+ "\"");
      }
  ...
  public static TokenizerFactory<Tree> tokenizerFactory(final TreeFactory tf, final TreeNormalizer tn, final Tokenizer stringTokenizer) {
