 ...
 * This data structure is backed by an {@link ArrayCoreMap}.
 *
 * @author dramage
 * @author rafferty
 */
public class CoreLabel implements CoreMap, Label, HasWord, HasTag, HasCategory,
HasContext, HasIndex, Serializable {
   ...
   * label.  Provides custom initialization if label implements CoreLabel
   * (default copy constructor) or if it implements MapLabel (copies map
   * values).
   */
  ...
  /** Wraps the given CoreMap. */
  private CoreLabel(ArrayCoreMap map) {
    this.map = map;
  }
   ...
   * It is guaranteed to not return null; if the key is not present, it returns
   * the empty String.  *However* it is only valid to call this method when
   * key is paired with a value of type String.
   * @param <KEY>
   * @param key
   * @return "" if the key is not in the map, the String value of the key otherwise
   */
