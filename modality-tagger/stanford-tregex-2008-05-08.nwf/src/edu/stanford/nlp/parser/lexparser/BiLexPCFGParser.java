...
public class BiLexPCFGParser implements KBestViterbiParser {
  ...
  protected void triggerHooks(Edge edge) {
    ...
    if (newR) {
      ...
      for (int i = 0; i < rules.length; i++) {
        BinaryRule br = rules[i];
        Collection realEdges = chart.getRealEdgesWithL(br.rightChild, edge.end);
        for (Iterator realI = realEdges.iterator(); realI.hasNext();) {
          Edge real = (Edge) realI.next();
  ...
  protected void triggerAllHooks(Edge edge) {
    ...
    if (newR) {
      ...
      for (Iterator<BinaryRule> rI = bg.ruleIteratorByLeftChild(edge.state); rI.hasNext();) {
        ...
        for (Iterator realI = edges.iterator(); realI.hasNext();) {
          Edge real = (Edge) realI.next();
          tempHook.start = real.start;
          tempHook.end = real.end;
          tempHook.state = br.parent;
  ...
  protected void processHook(Hook hook) {
    ...
    for (Iterator edgeI = edges.iterator(); edgeI.hasNext();) {
      Edge edge = (Edge) edgeI.next();
      combine(edge, hook);
    }
  }
  ...
  protected void postMortem() {
    ...
    int done = 0;
    while (!agenda.isEmpty()) {
      Item item = (Item) agenda.extractMin();
      done++;
      //if(done % (total/10) == 0)
