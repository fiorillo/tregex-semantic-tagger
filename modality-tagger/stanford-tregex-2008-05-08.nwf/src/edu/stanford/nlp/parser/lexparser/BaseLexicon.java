...
public class BaseLexicon implements Lexicon {
  ...
  public BaseLexicon(Options.LexOptions op) {
    ...
    if(op.uwModel == null) {
      op.uwModel = new BaseUnknownWordModel(op);
    }
    this.setUnknownWordModel(op.uwModel);
  }
...
//     }
        ...
        if (tNum > indexToStartUnkCounting) {
          ...
          if (seenCounter.getCount(iW) < 2) {
            ...
            if (DOCUMENT_UNKNOWNS) {
              ...
              EncodingPrintWriter.err.println("Unknown word/tag:\t" +
                                              wStr + "\t" + tStr,
                                              "UTF-8");
            }
  ...
  public float score(IntTaggedWord iTW, int loc) {
    ...
    if (seen) {
     ...
     pb_W_T = this.getUnknownWordModel().score(iTW, loc);
    }
  ...
  public void printLexStats() {
    ...
    System.out.println("Unseen counter: " + Counters.toString(unSeenCounter, nf));
  }
