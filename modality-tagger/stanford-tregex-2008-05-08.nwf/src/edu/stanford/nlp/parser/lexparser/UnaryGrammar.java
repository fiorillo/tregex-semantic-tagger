...
public class UnaryGrammar implements Serializable, Iterable<UnaryRule> {
  ...
  void makeCRArrays() {
    ...
    for (int i = 0; i < numStates; i++) {
      closedRulesWithP[i] = closedRulesWithParent[i].toArray(EMPTY_UNARY_RULE_ARRAY);
      closedRulesWithC[i] = closedRulesWithChild[i].toArray(EMPTY_UNARY_RULE_ARRAY);
    }
  }
  ...
  public void writeAllData(Writer w) {
    ...
    for (Iterator rI = ruleIterator(); rI.hasNext(); ) {
      out.println(rI.next().toString());
    }
    out.println("Unary closedRuleIterator");
    for (Iterator rI = closedRuleIterator(); rI.hasNext(); ) {
      out.println(rI.next().toString());
    }
    Numberer n = Numberer.getGlobalNumberer(stateSpace);
    ...
    for (int i = 0; i < numStates; i++) {
      ...
      for (Iterator rI = ruleIteratorByParent(i); rI.hasNext(); ) {
        out.print("  ");
        out.println(rI.next().toString());
      }
    }
    ...
    for (int i = 0; i < numStates; i++) {
      out.println(n.object(i));
      for (Iterator rI = closedRuleIteratorByParent(i); rI.hasNext(); ) {
        out.print("  ");
        out.println(rI.next().toString());
      }
    }
